...
public class BlueLongShot extends OpMode {
    ...
    private double collectionTargetX = 122.0;  // Will be set based on spike

    // Second collection spike
    private Pose secondSpike = null;
    private Pose secondSpikePost = null;
    private double secondCollectionTargetX = 122.0;
    ...
    public void init() {
        ...
        telemetry.addData("Start Pose", "X=%.1f Y=%.1f H=%.1f°",
                AutonConstants.blueLongStart.getX(),
                AutonConstants.blueLongStart.getY(),
                Math.toDegrees(AutonConstants.blueLongStart.getHeading()));
        telemetry.update();
    }
    ...
    private void buildPaths() {
        // Path from blueLongStart to blueLongScore (initial positioning) - FASTER
        ...
        // Path from blueLongScore to target spike
        scoreToSpike = follower.pathBuilder()
                .addPath(AutonConstants.blueLongScorePath(AutonConstants.blueLongScore))  // Placeholder
                .build();
    ...
    private void autonomousPathUpdate() {
        switch (pathState) {
            case 0:
                // Drive to blueLongScore first AND start shooter immediately
            ...
            case 10:
                ...
                if (!threeShots.isBusy()) {
                    ...
                    // Build path to blueGateRelease
                    Pose currentPose = follower.getPose();
                    com.pedropathing.paths.Path pathToGate = new com.pedropathing.paths.Path(
                            new com.pedropathing.geometry.BezierLine(currentPose, AutonConstants.blueGateRelease));
                    pathToGate.setLinearHeadingInterpolation(currentPose.getHeading(), AutonConstants.blueGateRelease.getHeading());
